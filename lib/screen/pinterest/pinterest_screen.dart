...
import 'generated_pinterest_screen.dart';
...
class _PinterestScreenState extends State<PinterestScreen> {
  ...
  void _generateQRCode() {
    ...
    // Kiểm tra URL có thuộc về Pinterest hay không
    if (!(uri.scheme == 'https' && uri.host.contains('pinterest.com'))) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please enter a valid Pinterest URL!"),
          duration: Duration(seconds: 2),
        ),
      );
      return;
    ...
    // Nếu URL hợp lệ và thuộc về pinterest, chuyển sang màn hình hiển thị QR code
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => GeneratedPinterestScreen(
          data: inputText,
        ),
      ),
    );
