...
import 'generated_facebook_screen.dart';
...
class _FacebookScreenState extends State<FacebookScreen> {
  ...
  void _generateQRCode() {
    ...
    if (!(uri.host.contains('facebook.com') || uri.host.contains('fb.com'))) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please enter a valid Facebook URL!"),
          duration: Duration(seconds: 2),
        ),
      );
      return;
    ...
    // Nếu URL hợp lệ và thuộc về Facebook, chuyển sang màn hình hiển thị QR code
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => GeneratedFacebookScreen(
          data: inputText,
        ),
      ),
    );
