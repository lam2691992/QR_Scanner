...
import 'generated_dropbox_sceen.dart';
...
// Kiểm tra URL có thuộc về Dropbox hay không
    if (!(uri.host.contains('dropbox.com'))) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Please enter a valid Dropbox URL!"),
          duration: Duration(seconds: 2),
        ),
      );
      return;
    ...
    // Nếu URL hợp lệ và thuộc về Dropbox, chuyển sang màn hình hiển thị QR code
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => GeneratedDropboxScreen(
          data: inputText,
        ),
      ),
    );
